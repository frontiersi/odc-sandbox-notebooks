...
class DataAccessApi:
    ...
    def get_full_dataset_extent(self, product, platform=None, longitude=None, latitude=None, time=None, output_crs=None, resolution=None, **kwargs):
        """
        ...
        """
        dataset = self.get_dataset_by_extent(
            product=product, platform=platform, longitude=longitude, latitude=latitude, time=time, output_crs=output_crs, resolution=resolution, dask_chunks={})

        if not dataset:
            return []
        return {'time': dataset.time, 'latitude': dataset.latitude, 'longitude': dataset.longitude}

    def get_datacube_metadata(self, product, platform=None, output_crs=None, resolution=None):
        """
        ...
        return self.get_query_metadata(product, platform=platform, output_crs=output_crs, resolution=resolution)
